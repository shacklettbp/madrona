...
static Optional<Window> makeWindow(const Backend &backend,
...
Backend::LoaderLib PresentationState::init(bool make_window)
{
#ifdef MADRONA_MACOS
...
#else
    Backend::LoaderLib loader_lib = [make_window]() -> Backend::LoaderLib {
        if (make_window) {
            return Backend::LoaderLib(nullptr, nullptr);
        } else {
            return Backend::loadLoaderLib();
        }
    } ();
#endif
...
vector<const char *> PresentationState::getInstanceExtensions(bool make_window)
{
    if (make_window) {
        uint32_t count;
